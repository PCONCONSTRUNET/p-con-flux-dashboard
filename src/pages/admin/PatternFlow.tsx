...
import { Activity, TrendingUp, TrendingDown, BarChart3, CheckCircle, XCircle, Clock, ChevronDown, ChevronUp, Flame, Zap } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in pb-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="rounded-2xl border border-primary/20 p-5 relative overflow-hidden backdrop-blur-xl" style={{ background: 'linear-gradient(135deg, hsla(187,100%,50%,0.06) 0%, hsla(240,6%,8%,0.9) 100%)' }}>
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full opacity-10 blur-3xl" style={{ background: 'hsl(187, 100%, 50%)' }} />
        ...
        <div className="rounded-2xl border border-emerald-500/20 p-5 relative overflow-hidden backdrop-blur-xl" style={{ background: 'linear-gradient(135deg, hsla(152,69%,50%,0.06) 0%, hsla(240,6%,8%,0.9) 100%)' }}>
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full opacity-10 blur-3xl" style={{ background: 'hsl(152, 69%, 50%)' }} />
          <div className="flex items-center justify-between mb-3">
        ...
        <div className="rounded-2xl border border-secondary/20 p-5 relative overflow-hidden backdrop-blur-xl" style={{ background: 'linear-gradient(135deg, hsla(345,100%,50%,0.06) 0%, hsla(240,6%,8%,0.9) 100%)' }}>
          <div className="absolute top-0 right-0 w-24 h-24 rounded-full opacity-10 blur-3xl" style={{ background: 'hsl(345, 100%, 50%)' }} />
          <div className="flex items-center justify-between mb-3">
        ...
        <div
          className={`rounded-2xl border p-5 relative overflow-hidden backdrop-blur-xl ${getWinRateBg(overallWinRate)}`}
          style={{ boxShadow: getWinRateGlow(overallWinRate) }}
        >
          <div className="flex items-center justify-between mb-3">
      ...
      <div className="space-y-3">
        ...
        {patternFlows.map((pf) => {
          ...
          return (
            <div
              key={pf.id}
              className="rounded-2xl border border-border/30 overflow-hidden transition-all duration-300"
              style={{
                background: 'linear-gradient(135deg, hsla(240,6%,10%,0.5) 0%, hsla(240,6%,6%,0.7) 100%)',
                boxShadow: isExpanded ? '0 8px 32px hsla(0,0%,0%,0.3)' : 'none',
              }}
            >
              {/* Card Header */}
              <button
                onClick={() => toggleExpand(pf.id)}
                className="w-full flex items-center justify-between p-4 lg:p-5 hover:bg-muted/10 transition-colors duration-200"
              >
                ...
                <div className="flex items-center gap-3 lg:gap-5">
                  ...
                  <div className="hidden sm:flex items-center gap-4">
                    ...
                    <div
                      className={`text-right px-4 py-2 rounded-xl border ${getWinRateBg(pf.winRate)}`}
                      style={{ boxShadow: getWinRateGlow(pf.winRate) }}
                    >
                  ...
                  <div className="flex items-center gap-2.5">
                    <div className={`flex items-center gap-1 text-[11px] font-display font-bold ${pf.isStreakGreen ? 'text-emerald-400' : 'text-secondary'}`}>
                      {pf.isStreakGreen ? <TrendingUp size={13} /> : <TrendingDown size={13} />}
                      {pf.streak}x
                    </div>
              ...
              {isExpanded && (
                <div className="border-t border-border/15 p-5 space-y-5" style={{ background: 'hsla(240,6%,6%,0.4)' }}>
                  ...
                  {/* Chart */}
                  <div className="rounded-xl border border-border/20 p-4" style={{ background: 'hsla(240,6%,8%,0.6)' }}>
                    <h4 className="font-display text-xs font-bold text-foreground mb-4 tracking-wide">Performance Diária</h4>
                    <ResponsiveContainer width="100%" height={200}>
                      <BarChart data={pf.dailyData} barGap={2}>
                        <defs>
                          <linearGradient id={`detailGreen-${pf.id}`} x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor="hsl(152, 69%, 55%)" stopOpacity={0.9} />
                            <stop offset="100%" stopColor="hsl(152, 69%, 55%)" stopOpacity={0.35} />
                          </linearGradient>
                          <linearGradient id={`detailLoss-${pf.id}`} x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor="hsl(345, 100%, 55%)" stopOpacity={0.9} />
                            <stop offset="100%" stopColor="hsl(345, 100%, 55%)" stopOpacity={0.35} />
                          </linearGradient>
                        </defs>
                        <CartesianGrid strokeDasharray="3 3" stroke="hsla(240,6%,30%,0.12)" />
                        <XAxis dataKey="day" tick={{ fontSize: 11, fill: 'hsla(240,6%,60%,0.6)', fontWeight: 600 }} axisLine={false} tickLine={false} />
                        <YAxis tick={{ fontSize: 10, fill: 'hsla(240,6%,60%,0.5)' }} axisLine={false} tickLine={false} />
                        <Tooltip content={<CustomTooltip />} cursor={{ fill: 'hsla(240,6%,50%,0.08)' }} />
                        <Bar dataKey="greens" name="Greens" fill={`url(#detailGreen-${pf.id})`} radius={[4, 4, 0, 0]} />
                        <Bar dataKey="losses" name="Losses" fill={`url(#detailLoss-${pf.id})`} radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5 px-1">
                    <span className="text-xs text-muted-foreground/60 font-display">Sequência atual:</span>
                    <div className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-display font-bold ${pf.isStreakGreen ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-secondary/10 text-secondary border border-secondary/20'}`}>
                      {pf.isStreakGreen ? <TrendingUp size={13} /> : <TrendingDown size={13} />}
