...
import { Bell, LayoutGrid, Clock, User, LogOut, Menu, X } from 'lucide-react';
import logo from '@/assets/logo.png';

const navItems = [
  { label: 'Alertas', icon: Bell, path: '/client' },
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-border bg-card">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display text-sm font-bold text-foreground">P-CON FLUX</span>
        </div>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-border flex justify-around py-2 z-40">
        {navItems.map(item => (
          <button
            key={item.path}
            onClick={() => navigate(item.path)}
            className={`flex flex-col items-center gap-0.5 px-3 py-1.5 text-xs transition-colors ${isActive(item.path) ? 'text-primary' : 'text-muted-foreground'}`}
          >
            <item.icon size={20} />
            {item.label}
          </button>
